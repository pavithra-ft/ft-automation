...
from Spiders.pgim_excel_extraction import get_fund_info, get_market_cap_values, get_fund_portfolio_values, \
    get_fund_code

try:
    os.chdir(r"C:\Users\pavithra\PycharmProjects\data_automation\excel files")
    sheet_names = ['February 2020 Core Equity Data', 'February 2020 Phoenix Data']
    files = [file for file in glob("*.xls")]
    for file in files:
        for sheet in sheet_names:
            ...
            market_cap_values = get_market_cap_values(dataframe)
            portfolio_values = get_fund_portfolio_values(dataframe)
            inst_code = get_inst_details("PGIM", database)
            ...
            fund_data, nav_data, marketcap_data, portfolio_data, sector_dataList = \
                put_into_iq_db(fund_code, fund_info, portfolio_values, market_cap_values, database, file_date)
