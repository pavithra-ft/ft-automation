...
def get_fund_record(file, sheet):
    ...
    try:
        ...
        app_logger.error('Exception raised in calculation: ' + str(error))
...
def put_fund_record(fund_data, fund_info):
    ...
    try:
        ...
        app_logger.error('Exception raised in queries: ' + str(error))
    finally:
...
def delete_files(fund_info):
    """
    This function removes the files once they are uploaded in the Database.

    :param fund_info: A class object of basic fund information
    """
    is_nav = query.is_nav_exist(fund_info.get_fund_code(), fund_info.get_reporting_date())
    is_fund_performance = query.is_fund_performance_exist(fund_info.get_fund_code(), fund_info.get_reporting_date())
    is_collaterals = query.is_collaterals_exist(fund_info.get_fund_code(), fund_info.get_reporting_date())
    is_fund_ratio = query.is_fund_ratio_exist(fund_info.get_fund_code(), fund_info.get_reporting_date())
    previous_1m_end_date = get_1m_date(fund_info.get_reporting_date())
    file_location = [fund_files[0] + "/" + fund_info.get_fund_code() + "_" +
                     str(fund_info.get_reporting_date()).replace("-", "") + ".xlsx"]
    if is_nav == 1 and is_fund_performance == 1 and is_collaterals == 1 and is_fund_ratio == 1:
        if os.path.exists(file_location[0]):
            os.remove(file_location[0])
    if not query.is_nav_exist(fund_info.get_fund_code(), previous_1m_end_date):
        shutil.move(file_location[0], data_missing_files[0] + "/" + fund_info.get_fund_code() + "_" +
                    str(fund_info.get_reporting_date()).replace("-", "") + ".xlsx")
