...
def get_alt_benchmark_info(fund_code):
    ...
    for details in alt_bm_details:
        alternate_benchmark = {"fund_code": details[0], "nav_start_date": str(details[1]), "alt_bm_index": details[2]}
...
def get_curr_price(current_date, alt_bm_info, database):
    curr_price_cursor = database.cursor()
    curr_price_query = "SELECT ip.index_price_close from index_prices ip where ip.index_code = '" + \
                       alt_bm_info['alt_bm_index'] + "' and year(ip.index_price_as_on_date) = year('" + str(
        current_date) \
...
def get_prev_price(current_date, alt_bm_info, database):
...
def get_prev_bm_nav(fund_code, current_date):
    ...
    prev_nav_query = "SELECT alt_benchmark_nav from fund_benchmark_nav where fund_code = '" + fund_code \
                     + "' and effective_end_date = '" + str(prev_1m_date) + "'"
...
def get_start_price(alt_bm_info, start_date_nav):
    nav_indexprice_cursor = database.cursor()
    nav_indexprice_query = "SELECT index_price_close from index_prices where index_code = '" + alt_bm_info[
...
def update_first_benchmark_nav(start_date_nav, alt_bm_info, fund_code, database):
    ...
    prev_index_price = get_start_price(alt_bm_info, start_date)
    curr_index_price = get_curr_price(start_end_date, alt_bm_info, database)
    prev_month_benchmark_nav = get_prev_bm_nav(fund_code, start_end_date)
    alt_benchmark_return = (curr_index_price / prev_index_price) - 1
    alt_benchmark_nav = round(prev_month_benchmark_nav * (1 + alt_benchmark_return), 6)
    print(fund_code, alt_benchmark_nav, start_end_date)
    put_into_fund_bm_nav(fund_code, alt_benchmark_nav, start_end_date, database)


def update_alt_benchmark_nav(current_date, alt_bm_info, fund_code, database):
    prev_index_price = get_prev_price(current_date, alt_bm_info, database)
    curr_index_price = get_curr_price(current_date, alt_bm_info, database)
    prev_month_benchmark_nav = get_prev_bm_nav(fund_code, current_date)
    alt_benchmark_return = (curr_index_price / prev_index_price) - 1
    alt_benchmark_nav = round(prev_month_benchmark_nav * (1 + alt_benchmark_return), 6)
    put_into_fund_bm_nav(fund_code, alt_benchmark_nav, current_date, database)
    print(fund_code, alt_benchmark_nav, current_date)


def put_into_fund_bm_nav(fund_code, alt_benchmark_nav, date, database):
    ...
    for fund_code in fund_code_list:
        ...
        for date in nav_dates_list:
            if date is nav_dates_list[0]:
                update_first_benchmark_nav(start_date_nav, alt_bm_info, fund_code, database)
                nav_dates_list.pop(0)
            else:
                current_date = date[0]
                update_alt_benchmark_nav(current_date, alt_bm_info, fund_code, database)
    database.commit()
