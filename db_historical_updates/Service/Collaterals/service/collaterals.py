...
def calc_collateral_data(fund_code, reporting_date):
    ...
    collateral_data.set_collateral_date(reporting_date[0] + datetime.timedelta(days=1))
    collateral_data.set_collateral_status('PUBLISHED')
    collateral_data.set_reporting_date(reporting_date)
    collateral_data.set_effective_start_date(reporting_date[0] + datetime.timedelta(days=1))
...
def get_collateral_data(fund_code_list):
    for fund_code in fund_code_list:
        ...
        for reporting_date in fund_dates:
            if not query.is_collaterals_exist(fund_code, reporting_date):
                collateral_data = calc_collateral_data(fund_code, reporting_date)
                try:
