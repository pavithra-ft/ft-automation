...
def calc_fund_ratios(fund_code, reporting_date):
    ...
    fund_ratio_data.set_sortino_ratio(((annualized_return - risk_free) / sigma))
...
def get_fund_ratios(fund_code_list):
    fund_code_list = query.get_fund_codes_fund_ratios(fund_code_list[0])
    fund_code_list.pop(0)
    for fund_detail in fund_code_list:
        fund_ratio_data = calc_fund_ratios(fund_detail[0], fund_detail[1])
        try:
