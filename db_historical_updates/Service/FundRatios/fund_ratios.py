...
from database.db_queries import get_fund_nav, get_benchmark_perf_1m, get_pe_ratio, get_fund_ratio_mcap, \
    get_nav_start_date, get_all_fund_return, get_risk_free_rate, get_fund_codes_fund_ratios, put_fund_ratios, \
    get_fund_portfolio
...
def calc_top5_pe_ratio(top5_holdings):
    ...
    for exposure in top5_holdings:
        exposure_values.append(exposure[1])
    calc_pe_ratio = []
...
def get_fund_ratios(fund_code, reporting_date):
    ...
    fund_nav = get_fund_nav(fund_code, reporting_date)[0][0]
    portfolio_values = get_fund_portfolio(fund_code, reporting_date)
    benchmark_perf_1m = get_benchmark_perf_1m(fund_code, reporting_date)
    fund_return_list = get_all_fund_return(fund_code, reporting_date)
    portfolio_sum = 0
