...
def calc_fund_performance(fund_code, reporting_date):
...
def calc_benchmark_perf_month(index_code, nav_start_date, curr_price, date):
    perf_mon = None
    price_mon = query.get_index_price_as_on_date(date, index_code)
    if price_mon:
        if nav_start_date <= date:
            perf_mon = round(((curr_price / float(price_mon[-1])) - 1), 4)
    return perf_mon


def calc_benchmark_perf_year(effective_end_date, index_code, nav_start_date, curr_price, date):
    perf_yr = None
    price_yr = query.get_index_price_as_on_date(date, index_code)
    date_power_yr = effective_end_date - date
    if price_yr:
        if nav_start_date <= date:
            perf_yr = round((((curr_price / float(price_yr[-1])) ** (365 / date_power_yr.days)) - 1), 4)
    return perf_yr


def calc_benchmark_perf_inception(effective_end_date, index_code, nav_start_date, curr_price):
...
def calc_benchmark_performance(fund_code, reporting_date):
    ...
    curr_price = float(query.get_index_price_as_on_date(effective_end_date, index_code)[-1])
    ...
    bm_perf_data.set_benchmark_perf_1m(calc_benchmark_perf_month(index_code, nav_start_date, curr_price, bm_date_1m))
    bm_perf_data.set_benchmark_perf_3m(calc_benchmark_perf_month(index_code, nav_start_date, curr_price, bm_date_3m))
    bm_perf_data.set_benchmark_perf_6m(calc_benchmark_perf_month(index_code, nav_start_date, curr_price, bm_date_6m))
    bm_perf_data.set_benchmark_perf_1y(calc_benchmark_perf_year(effective_end_date, index_code, nav_start_date,
                                                                curr_price, bm_date_1y))
    bm_perf_data.set_benchmark_perf_2y(calc_benchmark_perf_year(effective_end_date, index_code, nav_start_date,
                                                                curr_price, bm_date_2y))
    bm_perf_data.set_benchmark_perf_3y(calc_benchmark_perf_year(effective_end_date, index_code, nav_start_date,
                                                                curr_price, bm_date_3y))
    bm_perf_data.set_benchmark_perf_5y(calc_benchmark_perf_year(effective_end_date, index_code, nav_start_date,
                                                                curr_price, bm_date_5y))
    bm_perf_data.set_benchmark_perf_inception(calc_benchmark_perf_inception(effective_end_date, index_code,
...
def calc_alt_benchmark_performance(fund_code, reporting_date):
    ...
    alt_bm_data.set_alt_benchmark_perf_1m(calc_benchmark_perf_month(index_code, nav_start_date, curr_price,
                                                                    alt_1m_date))
    alt_bm_data.set_alt_benchmark_perf_3m(calc_benchmark_perf_month(index_code, nav_start_date, curr_price,
                                                                    alt_3m_date))
    alt_bm_data.set_alt_benchmark_perf_6m(calc_benchmark_perf_month(index_code, nav_start_date, curr_price,
                                                                    alt_6m_date))
    alt_bm_data.set_alt_benchmark_perf_1y(calc_benchmark_perf_year(effective_end_date, index_code, nav_start_date,
                                                                   curr_price, alt_1y_date))
    alt_bm_data.set_alt_benchmark_perf_2y(calc_benchmark_perf_year(effective_end_date, index_code, nav_start_date,
                                                                   curr_price, alt_2y_date))
    alt_bm_data.set_alt_benchmark_perf_3y(calc_benchmark_perf_year(effective_end_date, index_code, nav_start_date,
                                                                   curr_price, alt_3y_date))
    alt_bm_data.set_alt_benchmark_perf_5y(calc_benchmark_perf_year(effective_end_date, index_code, nav_start_date,
                                                                   curr_price, alt_5y_date))
    alt_bm_data.set_alt_benchmark_perf_inception(calc_benchmark_perf_inception(effective_end_date, index_code,
                                                                               nav_start_date, curr_price))
    return alt_bm_data
...
def table_records(fund_code, reporting_date):
    ...
    fund_perf_data = calc_fund_performance(fund_code, reporting_date)
...
def get_fund_performance(fund_code_list):
